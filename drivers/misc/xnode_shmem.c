...
/*
 * Optionally register as a bpf_mmio provider, so a BPF program holding an
 * open /dev/xnode_shmem fd can access the window as MMIO via
 * bpf_mmio_map(fd, ...) (e.g. to poke device registers in the same reserved
 * window). Off by default.
 */
static bool expose_mmio;
module_param(expose_mmio, bool, 0444);
MODULE_PARM_DESC(expose_mmio, "register as a bpf_mmio provider for the window");
...
/*
 * MMIO provider: let a BPF program touch this window via bpf_mmio_map(fd, ...)
 * where @fd is an open /dev/xnode_shmem. Authorization is the device node's
 * permissions (the caller must hold the fd); the resolver hands back the
 * window-relative physical address. A single provider (keyed on the shared
 * fops) covers every xnode_shmem device; the resolver picks the right window
 * from the fd's backing object.
 */
static int xnode_mmio_resolve(struct file *file, u64 offset, size_t size,
			      phys_addr_t *phys)
{
	struct xnode_shmem_file *f = file->private_data;
	struct xnode_shmem *xs = f->xs;

	if (offset >= xs->size || size > xs->size - offset)
		return -ERANGE;
	*phys = xs->base + offset;
	return 0;
}

static const struct bpf_mmio_provider xnode_mmio_provider = {
	.fops    = &xnode_shmem_fops,
	.name    = "xnode-shmem",
	.resolve = xnode_mmio_resolve,
};
...
static int __init xnode_shmem_init(void)
{
	...
	if (expose_mmio)
		bpf_mmio_register_provider(&xnode_mmio_provider);
	...
	if (base_param && size_param) {
		...
		if (err) {
			if (expose_mmio)
				bpf_mmio_unregister_provider(&xnode_mmio_provider);
