...
Importing the window into a *consumer-side* BPF program — to run the
ringbuf consumer in BPF rather than userspace — therefore requires a
page-backed window (a carveout heap, not `no-map`) on the consumer,
plus consumer-side coherency support (see Known gaps:
`USER_RINGBUF`).
...
- No selftests yet.
- Consumer-in-BPF (`USER_RINGBUF + BPF_F_DMABUF`) is rejected: it needs
  a page-backed window on the consumer and the consumer-side coherency
  protocol (invalidate before every producer-data read, clean after
  `consumer_pos`).
