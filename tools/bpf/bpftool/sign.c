		    ...
		    size_t max_sig_len)
{
	...
	__s64 len;
	X509 *x509;
	__s64 ret = -1; // Default return value in case of error
	...
	/*
	 * SECURITY: Changed "sha1" to "sha256". SHA-1 is insecure and
	 * should not be used for signatures.
	 */
...
cleanup:
	/* MEMORY FIX: Free all allocated OpenSSL objects */
	PKCS7_free(pkcs7);
	X509_free(x509);
