...
static void subtest_bad_flags(void)
{
	...
	/* map_extra without BPF_F_ARENA_BACKED: rejected, not ignored */
	{
		LIBBPF_OPTS(bpf_map_create_opts, opts, .map_extra = arena_fd);

		fd = bpf_map_create(BPF_MAP_TYPE_RINGBUF, "rb_noflag", 0, 0,
				    RB_SIZE, &opts);
		ASSERT_EQ(fd, -EINVAL, "map_extra without flag rejected");
	}
