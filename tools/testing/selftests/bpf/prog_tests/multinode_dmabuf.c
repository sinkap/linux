...
void test_multinode_dmabuf(void)
{
	...
	if (!ASSERT_OK_FD(open("/dev/dma_heap/system", O_RDONLY),
			  "dma_heap present"))
		return;
	...
	ASSERT_GE(reuse_dmabuf(skel->maps.arena, BPF_MAP_TYPE_ARENA, 4,
			       BPF_F_MMAPABLE | BPF_F_ARENA_CLEAN, 4, &arena_db),
		  0, "arena dma-buf map");
	int rb_fd = reuse_dmabuf(skel->maps.rb, BPF_MAP_TYPE_RINGBUF, 4096,
				 0, 3, &rb_db);
	int urb_fd = reuse_dmabuf(skel->maps.urb, BPF_MAP_TYPE_USER_RINGBUF,
				  4096, 0, 3, &urb_db);
	ASSERT_GE(rb_fd, 0, "ringbuf dma-buf map");
	...
	/* arena: the JIT must emit a clean after the arena store */
	if (test__start_subtest("arena_jit_clean")) {
		...
		ASSERT_TRUE(clflush, "clflush emitted after arena store");
