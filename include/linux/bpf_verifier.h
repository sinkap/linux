...
/* Forward declarations for verifier-internal types defined in
 * kernel/bpf/verifier_internal.h.  These appear only in pointer fields
 * of public types (bpf_verifier_env, bpf_verifier_state) and in
 * verifier-internal function signatures.
 */
 ...
 *
 * NOTE: this struct is part of the verifier ABI surface — it is
 * referenced by drivers/net/ethernet/netronome/nfp/bpf/verifier.c (the
 * NFP BPF offload driver dereferences ->stack[]).  Layout changes
 * therefore require a coordinated build of vmlinux + any out-of-tree
 * verifier module.
 */
...
/* NOTE: like bpf_func_state above, this struct is part of the verifier
 * ABI surface (the NFP BPF offload driver dereferences ->frame[] and
 * ->curframe).  Layout changes require coordinated rebuilds of vmlinux
 * and any out-of-tree verifier module.
 */
...
/* bpf_scc_backedge, bpf_scc_visit and bpf_scc_info are defined in
 * kernel/bpf/verifier_internal.h.  bpf_verifier_env only references
 * bpf_scc_info via a pointer field so a forward declaration (above) is
 * sufficient here.
 */
