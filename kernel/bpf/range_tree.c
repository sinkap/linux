 ...
 * The alloc/free of range_node-s is done via kmalloc(GFP_ATOMIC).
 *
...
int range_tree_clear(struct range_tree *rt, u32 start, u32 len)
{
	...
	while ((rn = range_it_iter_first(rt, start, last))) {
		if (rn->rn_start < start && rn->rn_last > last) {
			...
			/* Add a range */
			new_rn = kmalloc(sizeof(struct range_node), GFP_ATOMIC | __GFP_NOWARN);
			if (!new_rn)
				return -ENOMEM;
...
int range_tree_set(struct range_tree *rt, u32 start, u32 len)
{
	...
	if (left && right) {
		...
		left = kmalloc(sizeof(struct range_node), GFP_ATOMIC | __GFP_NOWARN);
		if (!left)
			return -ENOMEM;
		left->rn_start = start;
		left->rn_last = last;
