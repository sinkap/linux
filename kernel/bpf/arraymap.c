...
static void array_map_free(struct bpf_map *map)
{
	...
	if (map->arena_backing) {
		 ...
		 * private meta pages + the arena's value pages -- @array
		 * itself lives in the meta pages. Read everything teardown
		 * needs (the backing pointer, the private pages) BEFORE
		 * dropping the mapping: after vunmap() any access through
		 * @array/@map faults on the dead mapping.
		 */
		struct bpf_map_arena_backing *ab = map->arena_backing;
