 ...
 * lazily created faux device serves all imports.
 */
static struct faux_device *bpf_dmabuf_fdev;
...
static struct device *bpf_dmabuf_importer_dev(void)
{
	...
	if (!bpf_dmabuf_fdev) {
		...
		fdev = faux_device_create("bpf_dmabuf", NULL, NULL);
		if (fdev &&
		    dma_coerce_mask_and_coherent(&fdev->dev, DMA_BIT_MASK(64))) {
			faux_device_destroy(fdev);
			fdev = NULL;
		}
		bpf_dmabuf_fdev = fdev;
	}
	if (bpf_dmabuf_fdev)
		dev = &bpf_dmabuf_fdev->dev;
